        ...
        with self.add_file_widgets['output']:
            ...
            try:
                ...
                self.generator.files = [
                    f for f in self.generator.files
                    if f['file_name'] != file_to_remove
                ]
                self.generator.fields = [
                    f for f in self.generator.fields
                    if f['file_name'] != file_to_remove
                ]
    ...
    def show_file_config(self, file_name: str) -> widgets.VBox:
        ...
        if is_static:
            config_widgets.append(
                widgets.HTML("<i style='color: #666;'>Note: Static file configuration is protected.</i>")
            )
